...
param(
    ...
    [string]$action         = "Deploy", # "GetInfo"
    [string]$username       = "jsonbuser2", # do not prefix with the webapp name
    [string]$password       = "",
    [string]$webAppName     = "todo-web-app",
...
function main() {
    ...
    if($password -eq "") {
        $password = read-host -promt "Password:(Usual as for cat but using $webAppName)"
    }
